...
class BoardStructure:
    ...
    Attributes:
        ...
        vector_space_header (list): String identifiers for board vector representation.
        ...
        board_position (list[float]): Figure-relative rectangle [left, bottom, width, height]
            used when rendering the main board axes with matplotlib. Taken from
            the provided `BoardLayout` and useful for external plotting utilities.
        bar_positions (list[list[float]]): List of four figure-relative rectangles
            used to render per-player resource bars. Each entry follows
            [left, bottom, width, height] figure coordinates and is sourced
            from the `BoardLayout` configuration.
        text_box (list[float]): Figure-relative rectangle used for chat/text
            overlays (e.g. `ChatWindow`) when producing visualizations. Also
            provided by the `BoardLayout` instance.
    ...
    def __init__(self, board_layout: BoardLayout = BoardLayout()) -> None:
        ...
        self.path_to_ffmpeg = board_layout.path_to_ffmpeg

        
        self.edge_edge_matrix = np.array([
            [1 if self._check_if_edges_are_connected(r,c) else 0 for c in range(self.no_of_edges)]
        ...
        self.trade_player_actions = [1 if self.index_to_action(index)[0] == 'trade_player' else 0  for index in range(self.mask_indices['length'])]
        self.board_position = board_layout.board_position
        self.bar_positions = board_layout.bar_positions
        self.text_box = board_layout.text_box


    def _calc_coordinates(self) -> tuple[list, list, list]:
        ...
        Args:
            None: Uses instance attributes (_rings, no_of_nodes, etc.)
    ...
    def _calc_neighbour_nodes_for_nodes(self):
        """Calculate direct neighbor nodes for each node in the hexagonal grid.
        
        This method builds a mapping of node adjacencies by examining all edges
        in the board structure. For each edge that connects two nodes, those
        nodes are marked as neighbors of each other.
        
        Returns:
            list[set]: List where each index represents a node, and the value
                is a set containing the indices of all directly connected nodes.
                
        Example:
            If edge connects nodes 5 and 8, then:
            - neighbour_nodes_for_nodes[5] will contain 8
            - neighbour_nodes_for_nodes[8] will contain 5
                
        Note:
            This is a private method called during initialization. The result
            is used for path finding, street connectivity validation, and
    ...
    def _check_if_edges_are_connected(self,r,c):
        """Helper function to determine if two edges are adjacent.
        
        Args:
            r (int): Index of first edge
            c (int): Index of second edge
            
        Returns:
            bool: True if edges share exactly one node but are not identical
        """
    ...
    def _calc_secondary_neighbour_nodes_for_nodes(self):
        """Calculate secondary (distance-2) neighbor nodes for each node.
        
        This method finds nodes that are connected through exactly one intermediate
        node, creating a "neighbors of neighbors" relationship. Secondary neighbors
        are nodes that can be reached in exactly 2 steps through the graph.
        
        Returns:
            list[set]: List where each index represents a node, and the value
                is a set containing indices of all nodes at distance 2.
                
        Example:
            If nodes 1-5-9 are connected in sequence:
            - Node 1's direct neighbor is 5
            - Node 1's secondary neighbor is 9 (through 5)
            - Node 9 would not be in node 1's direct neighbors
                
        Note:
            This is a private method called during initialization. Secondary
            neighbors are useful for strategic planning and validating building
            placement rules in Catan.
        """
    ...
    def _calc_neighbour_nodes_for_tiles(self) -> list[set]:
        """Calculate which nodes are adjacent to each tile in the hexagonal grid.
        
        This method determines the node adjacency relationships for each tile
        (hexagon) on the board. Each tile is surrounded by exactly 6 nodes
        at its corners. This mapping is essential for determining resource
        distribution when dice are rolled.
        
        The calculation is performed ring by ring, starting from the center
        tile and working outward. For each ring, the method identifies which
        nodes form the corners of each tile in that ring.
        
        Returns:
            list[set]: List where each index represents a tile, and the value
                is a set containing the indices of all 6 nodes that border
                that tile.
                
        Example:
            For a center tile (index 0), the returned set might be {0,1,2,3,4,5}
            indicating that nodes 0-5 form the corners of the center hexagon.
                
        Note:
            This is a private method called during initialization. The result
            is used for resource distribution calculations and determining
            which players receive resources when specific dice values are rolled.
        """
    ...
    def polar_to_node(self, polar) -> int:
        ...
        The formula used is:
        node_index = 6*polar[0]*(polar[0]-1) + polar[1]
        ...
        Args:
            polar (tuple): A tuple representing the polar coordinates (ring, position).
    ...
    def polar_to_edge(self, polar) -> int:
        ...
        > edge_index = 9*polar[0]*polar[0] - 3*polar[0] + polar[1]
    ...
    def get_vector_indices(self) -> dict:
        """Create comprehensive mapping for board state vector indices.
        
        Generates a dictionary that maps state vector component names to their
        corresponding indices in the flattened board state representation.
        This mapping is essential for AI training as it provides structured
        access to specific components of the board state vector.
        
        The vector includes the following components in order:
        1. Game metadata (turns remaining)
        2. Player rankings and victory points
        3. Node occupation status (who owns each intersection)
        4. Edge occupation status (who owns each road)
        5. Player resource hands (cards held by each player)
        
        Returns:
            dict: Dictionary containing index mappings with keys:
                - Individual component names (str): Map to specific indices (int)
                - 'length': Total length of the state vector
                - Category keys: Lists of indices for grouped components:
                    - 'turns': [turns_before_end_index]
                    - 'ranks': [rank indices for all players]
                    - 'values': [victory point indices for all players]
                    - 'nodes': [node occupation indices]
                    - 'edges': [edge occupation indices]  
                    - 'hands': [resource hand indices for all players]
                    - 'hand_for_player': [resource indices per player]
                    
        Example:
            >>> board = BoardStructure()
            >>> indices = board.get_vector_indices()
            >>> 
            >>> # Access specific components
            >>> turns_idx = indices['turns_before_end']
            >>> player_a_score_idx = indices['value_A']
            >>> node_5_owner_idx = indices['node_5']
            >>> 
            >>> # Access component groups
            >>> all_node_indices = indices['nodes']
            >>> player_c_resources = indices['hand_for_player'][2]  # Player C (index 2)
            >>> 
            >>> # Get vector size
            >>> vector_size = indices['length']
            
        Note:
            This method is called during initialization. The mapping must be
            consistent with the board state vectorization used in game logging
    ...
    def get_action_mask_indices(self) -> dict:
        """Create comprehensive mapping from action identifiers to indices.
        
        Generates a dictionary that maps action identifier strings to their
        corresponding numerical indices in the action space. This mapping is
        essential for converting between human-readable action descriptions
        and the numerical indices used by AI models.
        
        The method creates mappings for all action types included in the
        `included_actions` list, including individual actions and grouped
        action categories for efficient batch operations.
        
        Returns:
            dict: Dictionary containing action mappings with keys:
                - Individual action keys (str): Map to specific indices (int)
                - 'length': Total number of actions in the space
                - Category keys ('streets', 'villages', etc.): Lists of indices
                    for each action category
                    
        Example:
            >>> board = BoardStructure()
            >>> indices = board.get_action_mask_indices()
            >>> 
            >>> # Get index for specific action
            >>> street_5_index = indices['street_5']
            >>> 
            >>> # Get all street action indices  
            >>> all_street_indices = indices['streets']
            >>> 
            >>> # Get total action space size
            >>> total_actions = indices['length']
            
        Note:
            This method is called during initialization. The resulting mapping
            must be consistent with `create_mask_space_header()` and is used
            by `action_to_index()` for efficient action lookup.
        """
    ...
    def get_trade_options_array(self) -> np.array:
        """Convert trade options list to numpy array for efficient processing.
        
        Transforms the list of possible trade combinations into a 2D numpy
        array where each row represents a trade option. This format is more
        efficient for vectorized operations and AI model processing.
        
        Each trade option consists of two resource indices:
        - Column 0: Resource being given away
        - Column 1: Resource being received
        
        Returns:
            np.ndarray: 2D array of shape (n_trades, 2) with dtype int8.
                Each row represents one possible trade as [give_resource, get_resource].
                Resource indices correspond to positions in `resource_types`.
                
        Example:
            >>> board = BoardStructure()
            >>> trades = board.get_trade_options_array()
            >>> print(trades.shape)  # (30, 2) for 6 resources
            >>> print(trades[0])     # [0, 1] - trade resource 0 for resource 1
            >>> 
            >>> # Use for vectorized trade validation
            >>> player_resources = np.array([3, 1, 2, 0, 1, 4])
            >>> can_trade = player_resources[trades[:, 0]] > 0
            
        Note:
            This method is called during initialization. The array provides
            an efficient representation for AI models to process all possible
            trade options simultaneously.
        """
    ...
    def create_mask_space_header(self):
        """Create comprehensive header list for action mask space.
        
        Generates an ordered list of action identifiers that corresponds to
        the action space used for AI training. This header defines the mapping
        between action indices and their string representations.
        
        The header includes all action types specified in `included_actions`:
        - Pass/do-nothing action (always first)
        - Street building actions for each edge
        - Village building actions for each node  
        - Town building actions for each node
        - Trading actions for each possible resource combination
        
        Returns:
            list[str]: Ordered list of action identifier strings. Each string
                uniquely identifies an action that can be taken in the game.
                The index of each string in this list corresponds to the
                action index used in AI training.
                
        Example:
            >>> board = BoardStructure()
            >>> headers = board.create_mask_space_header()
            >>> print(headers[:5])
            ['pass', 'street_0', 'street_1', 'street_2', 'street_3']
            >>> 
            >>> # Use with action indexing
            >>> action_index = 10
            >>> action_name = headers[action_index]
                
        Note:
            This method is called during initialization and the result is
            stored in `mask_space_header` attribute. The order must remain
            consistent with `action_to_index()` and `index_to_action()` methods.
        """
