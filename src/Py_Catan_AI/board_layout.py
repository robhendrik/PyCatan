...
class BoardLayout:
    ...
    Attributes:
        ...
        path_to_ffmpeg (str): File path to the ffmpeg executable for video generation.
        board_position (list[float]): Figure-relative rectangle [left, bottom, width, height]
            used for the main board axes when rendering with matplotlib. Default
            is [0.15, 0.15, 0.4, 0.7].
        bar_positions (list[list[float]]): List of four figure-relative rectangles
            used to render the per-player resource bars. Each entry is a
            [left, bottom, width, height] list. Defaults target upper-left,
            upper-right, lower-left and lower-right corners of the figure.
        text_box (list[float]): Figure-relative rectangle used for an on-figure
            chat or text box overlay. Default is [0.65, 0.05, 0.45, 0.9].
    ...
    resource_type_names: list[str] = field(default_factory=lambda: ['Brick', 'Desert', 'Grain', 'Ore', 'Sheep', 'Wood'])  # Human-readable resource names
    ...
    plot_colors_players: list[str] = field(default_factory=lambda: ['blue','green','red','yellow','purple','pink','orange'])  # Player colors for visualization
    plot_labels_for_resources: list[str] = field(default_factory=lambda: ['Brick', 'Desert','Grain', 'Ore', 'Sheep', 'Wood'])  # Human-readable resource names
    ...
    path_to_ffmpeg: str = r"C:\ffmpeg\ffmpeg-8.0-full_build\bin\ffmpeg.exe"
    board_position: list[float] = field(default_factory=lambda: [0.15, 0.15, 0.4, 0.7])  # Board plot position
    bar_positions: list[list[float]] = field(default_factory=lambda: [[0.05, 0.75, 0.1, 0.2],
                                                                    [0.55, 0.75, 0.1, 0.2],
                                                                    [0.05, 0.15, 0.1, 0.2],
                                                                    [0.55, 0.15, 0.1, 0.2]])  # Player bar plot positions
    text_box: list[float] = field(default_factory=lambda: [0.65, 0.05, 0.45, 0.9])  # Text box plot position
