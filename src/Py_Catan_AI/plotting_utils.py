"""Plotting utilities for PyCatan.

This module contains lightweight helpers to render a Catan board and
per-player information using matplotlib. It is intentionally minimal and
designed for debugging, demo videos, and interactive inspection of
`GameLog` entries.

Key components:
    - `ChatWindow`: small in-figure chat box to render recent messages.
    - `plot_board_positions_with_indices_from_vector`: draw a full board
        representation (tiles, nodes, edges) annotated with indices and
        per-player ownership inferred from a state vector.
    - `video_from_log`: create a short MP4 showing board progression from
        a `GameLog` wrapper.

Notes:
    - The functions expect a `BoardStructure` instance describing board
        coordinates and plotting metadata (colors, headers, limits).
    - The module uses matplotlib animation and writes video files via
        ffmpeg if `BoardStructure.path_to_ffmpeg` is configured.
...
FIGSIZE = (15, 10)
DEFAULT_PATH_TO_DOCS = 'Py_Catan_AI.visuals'



class ChatWindow:
    """Simple in-figure chat/message box for game UI overlays.

    The `ChatWindow` draws a small stack of message bubbles inside a
    reserved axes rectangle. It is intended to be used alongside the
    board axes so that chat does not get cleared when the board is
    redrawn.

    Args:
        fig (matplotlib.figure.Figure): Figure to attach the chat window to.
        game: Game object with a `.structure` attribute providing
            `plot_colors_players` used for bubble colors.
        rect (list): Axes rectangle [left, bottom, width, height] in
            figure coordinates.
        max_messages (int): Maximum number of messages to display.
    """

    def __init__(self, fig, game, rect=None, max_messages=10):
        if rect is None:
            rect = game.structure.text_box
    ...
    def add_comment(self, player_id, player_name, text):
        """Add a short comment to the chat window and refresh drawing.

        Args:
            player_id (int): Player numeric id (used to pick a color).
            player_name (str): Human-readable player name to display.
            text (str): Comment text (may be wrapped).
        """
...
def _plot_board_indicators(structure: BoardStructure, dice: int, ax: any) -> None:
    """
    Plot the board indicators (i.e., the nodes, edges and tile with their index).
    The tiles with the number of the last dice roll are highlighted.

    Args:
        structure (BoardStructure): The game structure containing rules and indices.
        dice (int): The value of the last dice roll.
...
def _add_players_to_board(structure: BoardStructure, ax: any, vector: np.ndarray) -> None:
    """
    Plot the streets, villages, towns of the players on the board.
    ...
    """
    # Fetch the colors we use to plot the players
    colors = structure.plot_colors_players

    # Create a small helper class to hold the data for each player
    @dataclass
    class Place:
        streets: list
        villages: list
    ...
    # For each player plot their streets, villages, and towns. The coordinates are taken from 
...
def plot_board_positions_with_indices_from_vector(structure, input_vector, names, active_player=0, info=0, fig=None):
    """Render the board and per-player resource bars from a state vector.

    This function draws a full board representation annotated with tile
    indices, node/edge indices and the current ownership of streets,
    villages and towns extracted from ``input_vector``. It also draws
    four small resource bars (one per player) which display current
    hand counts and a small score/street-length summary.

    Args:
        structure (BoardStructure): Board metadata (coordinates, headers,
            colors and vector index mappings).
        input_vector (np.ndarray): Game state vector (game-order or
            active-player-first depending on caller — the function uses
            values as-is).
        names (list[str]): Player display names in game order.
        active_player (int): Active player id (used for title/annotation).
        info (dict): Dict-like metadata matching fields created by
            `create_log_entry` (expects keys like 'dice result', 'stage',
            'rounds', 'score', 'street_length').
        fig (matplotlib.figure.Figure, optional): If provided the plot is
            drawn into this figure; otherwise a new figure is created.

    Returns:
        matplotlib.figure.Figure: Figure containing the rendered board and
            resource bars.
    """
    # === Layout ===
    try:
        board_position = structure.board_position
    except:
        board_position = [0.15, 0.15, 0.4, 0.7]
    try:
        bar_positions = structure.bar_positions
    except:
    ...
    ax_board.axis('off')
    _plot_board_indicators(structure, info['dice result'], ax=ax_board)
...
def video_from_log(game_log: any,filename="game_progress.mp4"):
    """Create an MP4 visualizing the game progression from a `GameLog`.

    This helper builds an animation where the first frame is a simple
    title/intro image and subsequent frames render board snapshots and a
    chat window with the move or comment for each recorded log entry.

    Args:
        game_log: `GameLog` wrapper object (must have `structure`, `game`
            and `log` attributes).
        filename (str): Output filename for the MP4 file.

    Notes:
        - The function expects `structure.path_to_ffmpeg` to point to a
          valid ffmpeg binary for the animation writer. It will set the
          matplotlib rcParam accordingly.
        - The function is not optimized for speed; it recreates plotting
          state for each frame which is acceptable for short demo videos.
    """
    ...
    try:
        ...
        text_box = [0.65, 0.05, 0.45, 0.9]
    plt.rcParams['animation.ffmpeg_path'] = structure.path_to_ffmpeg
    game = game_log.game
    ...
    fig = plt.figure(figsize=FIGSIZE)
    chat = None
    path_to_intro_img = files(DEFAULT_PATH_TO_DOCS).joinpath('VideoTitleHeader.png')
    ...
    def update(i):
        ...
        if i == 0:
            ...
            ax.axis("off")
            ax.text(
                0.5, 0.9, "PyCatan, the AI version",
        ...
        plot_board_positions_with_indices_from_vector(
